...
Developers everywhere are buzzing about its game-changing potential for faster builds. Recently saw a X tweet claiming, they will not go for claude code as codex is better at planning first.
...
Enter Rube, the universal MCP hub and in this short blog, let's see how you can use it to supercharge your OpenAI Codex workflow across IDE, CLI, and cloud.

---

## How to setup MCP for Codex

The benefit of codex is it can run in all 3 modes:

- **GUI Mode**: You can unstall the Codex extension in VS Code, Cursor, or Windsurf. It reads your workspace; edits code and runs tasks right in your favorite IDE.
...
- **Cloud Mode**: Power users can Delegate heavy tasks to Codex Cloud from IDE or GitHub (@codex tags), freeing them to focus while it handles builds remotely. This is a unique edge over rivals.

- **Codex App**: The new Codex app. A very beautiful UI thats very intuitive to work with.

But to use rube, all requires a bit of setup, here is how to do it for all modes!

### 1. Setup MCP in Codex GUI Mode

I thought connecting remote MCP servers to Codex extension was easy, but it's a major [issue](https://github.com/openai/codex/issues/6465)., an open one right now on GitHub.
So, here's the clean workaround I found to use MCP with extension:

- Go to [Rube](https://rube.app/), signup/login or whatever MCP servers you want to use.
...
- install [Codex extension](https://marketplace.visualstudio.com/items?itemName=OpenAI.chatgpt) for VS Code and login.
...
To verify if mcp server is enabled or not, paste the following in prompt box:
...
Now let’s setup the same for CLI.

### 2. Setup MCP in Codex CLI Mode

Terminal lover’s here is how to setup mcp in codex cli mode . Multiple method exists, but I like the `config.toml` approach, feels native.

- Install the codex cli using : `npm i -g @openai/codex`  (make sure you have `npm` install)

- Initialize a project root directory. I used `codex-test`

- Create a folder named `codex` using `mkdir` . 

- Inside it create a new file called `config.toml` and paste the following text from below and save it!
...
> auth-token can be fetched same as in previous step

or you can cut the hassle with new update: (only works with native oAuth supported mcp). 
...
Now head to root of file and run `codex`, login and use `/mcp` , if it shows the output as give, you are done.
...
Now time to setup MCP for codex using cloud mode.

### 3. Setup MCP in Codex Cloud Mode

Codex allows delegating task to cloud instance and let developer focus on another task.

Sadly, this is long planned yet not shipped. The blockers being: secure tool proxying, credential forwarding, deterministic replays and more.

A smart way is to use mcp process locally and then delegate responses + rest of work to cloud.

### 4. Setup MCP in Codex App

- Click on Settings gear icon on the bottom left.

- Go to MCP Servers. You’ll see some servers which you can authenticate and use.
...
So, with this we are all set to look at few use cases of codex!

---

## Use Cases of Codex + MCP

Codex is mainly meant for coding, but with [Rube MCP](/25df261a6dfe804791b6fe4a3709d774) it can handle variety of task. Here are 3 use cases I like to use as a developer.
...
This gives me rough idea of where I left and start from there.  No more mundane checks.

But the use cases, don’t stop here.

### 2. Creating Changelogs / Project Report on the fly

Integrations used: [GitHub](https://composio.dev/toolkits/github), [Google Doc](https://composio.dev/toolkits/googledoc)
...
By far, I have shared, productivity stuff, time for an actual development use case!
...
But now I fire up codex and ask it to extract design tokens for me that I can use to replicate the frontend in similar style.
...
Ofc, I make few changes here and there before implementing, but this now codex + rube combo make’s life so easier.
...
Many developers love to write devlogs (summary of what they did) while building product. But with codex + mcp , you can analyze each dev log day wise basis and create a blog that you can then share to the world. Similar to how major big companies. 
...
By default, codex doesn’t have access to google docs, but MCP helps here. So, we let codex handle the generation process and rube handled the google docs. 
...
Even with vibe coding pushing software dev forward, most people still treat Codex / MCP like a magic box. That’s the actual problem.
...
Going forward, this becomes new normal. Using these tools well won’t be optional.
